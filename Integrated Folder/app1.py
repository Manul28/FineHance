...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///tracker3.db'
...
def signup():
    ...
    if request.method == 'POST' and form.validate():
        ...
        # Create new user
        new_user = User(first_name=first_name, email=email, username=username, password=password)
...
def addTransactions():
    if request.method == 'POST':
        ...
        salary = request.form['salary']
        new_transaction = Transaction(user_id=session['user_id'], salary=salary,amount=amount, description=description, category=category)
        ...
        if len(transactions) == 0:
             ...
             totalSavings = transactions[0].salary - totalExpenses
        return render_template('addTransactions.html', totalSavings=totalSavings,totalExpenses=totalExpenses, transactions=transactions)
...
def excelForm():
    ...
    df = pd.DataFrame([(t.id, t.user_id,t.salary,t.amount, t.description, t.category, t.date) for t in transactions],
                      columns=['ID', 'User ID','Salary','Amount', 'Description', 'Category', 'Date'])
